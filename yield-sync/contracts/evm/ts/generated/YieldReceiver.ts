...
export class YieldReceiver {
  ...
  vaultStates(
    runtime: Runtime<unknown>,
  ): readonly { vault: `0x${string}`; stationId: string; phase: number; lastRebasedAt: bigint }[] {
    const callData = encodeFunctionData({
      abi: YieldReceiverABI,
      functionName: 'vaultStates' as const,
    })
    ...
    return decodeFunctionResult({
      ...
      functionName: 'vaultStates' as const,
      data: bytesToHex(result.data),
    }) as readonly { vault: `0x${string}`; stationId: string; phase: number; lastRebasedAt: bigint }[]
  }
